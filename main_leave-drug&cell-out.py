...
if __name__ == "__main__":
    ...
    sensitive = np.load(os.path.join(args.data_dir, "filtered_sensitivity_index_list.npy"),
                        allow_pickle=True).astype(int)
    resistant = np.load(os.path.join(args.data_dir, "filtered_resistant_index_list.npy"),
                        allow_pickle=True).astype(int)
    ...
    drug_graphs = torch.load(os.path.join(args.data_dir, "drug_graphs.pt"))
    ...
    if not has_saved_checkpoint:
        print("⚠️ No valid validation-AUC checkpoint was saved. Saving current model before test.")
        torch.save(model.state_dict(), save_path)
