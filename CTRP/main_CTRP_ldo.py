...
if __name__ == "__main__":
    ...
    rna_seq = torch.as_tensor(rna_seq, dtype=torch.float32, device=device)
    gene_cnv = np.load(os.path.join(args.data_dir, "cnv.npy"), allow_pickle=True).astype(float)
    gene_cnv = torch.as_tensor(gene_cnv, dtype=torch.float32, device=device)

    sensitive = np.load(os.path.join(args.data_dir, "sensitive.npy"), allow_pickle=True).astype(int)
    resistant = np.load(os.path.join(args.data_dir, "resistant.npy"), allow_pickle=True).astype(int)
    ...
    all_data = np.vstack((sensitive, resistant))
    unique_drugs = np.array(sorted(set(all_data[:, 1])))
    rng = np.random.RandomState(args.seed)
    rng.shuffle(unique_drugs)
    n_d = len(unique_drugs)
    train_drugs = set(unique_drugs[:int(n_d * 0.8)])
    val_drugs = set(unique_drugs[int(n_d * 0.8):int(n_d * 0.9)])
    test_drugs = set(unique_drugs[int(n_d * 0.9):])

    train_index_np = all_data[np.array([r[1] in train_drugs for r in all_data])]
    val_index_np = all_data[np.array([r[1] in val_drugs for r in all_data])]
    test_index_np = all_data[np.array([r[1] in test_drugs for r in all_data])]
    ...
    drug_graphs = torch.load(
        os.path.join(args.data_dir, "drug_graphs.pt"), map_location=device, weights_only=False
    )
