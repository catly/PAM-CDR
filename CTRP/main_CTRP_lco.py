...
if __name__ == "__main__":
    ...
    unique_cells = np.array(sorted(set(all_data[:, 0])))
    rng.shuffle(unique_cells)
    n_c = len(unique_cells)
    train_cells = set(unique_cells[:int(n_c * 0.8)])
    val_cells = set(unique_cells[int(n_c * 0.8):int(n_c * 0.9)])
    test_cells = set(unique_cells[int(n_c * 0.9):])

    train_index_np = all_data[np.array([r[0] in train_cells for r in all_data])]
    val_index_np = all_data[np.array([r[0] in val_cells for r in all_data])]
    test_index_np = all_data[np.array([r[0] in test_cells for r in all_data])]
