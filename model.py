...
class Drugcell(nn.Module):
    def __init__(self, num_tpm, num_genecnv, node_dim, drug_feat):
        ...
        self.alpha_logit = nn.Parameter(torch.tensor(0.7))
        self.mlp_end = MLP_end(144, 2)
    ...
    def forward(
        ...
        rna_seq = torch.nan_to_num(rna_seq, nan=0.0, posinf=0.0, neginf=0.0)
        gene_cnv = torch.nan_to_num(gene_cnv, nan=0.0, posinf=0.0, neginf=0.0)
        rna_seq1 = self.mlp_1(rna_seq)
        gene_cnv1 = self.mlp_2(gene_cnv)
        alpha = self.alpha_logit
        beta = 1.0 - alpha
        ...
        for i in range(len(drug_fea)):
            ...
            model_device = rna_seq.device
            prompt_text = prompt_text.to(model_device)
            ...
            gcn_fea = self.graph_prompt_CrossEncoder(gcn_fea, prompt_emb, prompt_mask)
            gcn_fea = torch.unsqueeze(torch.max(gcn_fea, dim=0)[0], dim=-1)

            drug_morgan = drug_graphs[i].drug_morgan
            drug_morgan = torch.as_tensor(
                drug_morgan,
                dtype=torch.float32,
                device=model_device,
            )
            ...
            fp_fea = self.fp_prompt_CrossEncoder(fp_fea, prompt_emb, prompt_mask)
        ...
        drug_fea, attn1, attn2 = self.drug_cross_attn(drugfea_graph, drugfea_fp)
...
class OmicsEncoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        bottleneck_size = min(128, hidden_size)
...
class MLP_1(OmicsEncoder):
    pass
class MLP_2(OmicsEncoder):
    pass

class Graph_prompt_CrossEncoder(nn.Module):
    def __init__(self, text_dim=768, graph_dim=72, n_heads=3):
        super().__init__()
        assert graph_dim % n_heads == 0, "graph_dim must be divisible by num_heads."
    ...
    def forward(self, gcn_fea, prompt_emb, prompt_mask=None):

        prompt_proj = self.text_proj(prompt_emb.squeeze(0))

        key_padding_mask = None
        if prompt_mask is not None:
            key_padding_mask = prompt_mask.eq(0)

        attn_output, _ = self.cross_attn(
            query=gcn_fea.unsqueeze(0),
...
class FP_prompt_CrossEncoder(nn.Module):
    def __init__(self, text_dim=768, fp_dim=36, n_heads=3):
        super().__init__()
        assert fp_dim % n_heads == 0, "fp_dim must be divisible by num_heads"
    ...
    def forward(self, fp_fea, prompt_emb, prompt_mask=None):
...
class Cell_CrossAttention(nn.Module):
    ...
    def forward(self, feat1, feat2):
        ...
        q1 = self.q_proj_1(feat1).unsqueeze(1)
        ...
        fused = torch.cat([out1, out2], dim=-1).squeeze(1)
...
class Drug_CrossAttention(nn.Module):
    ...
    def forward(self, drugfea_graph, drugfea_fp):
        ...
        out1, attn1 = self.attn1(q1, k1, v1)
        ...
        out2, attn2 = self.attn2(q2, k2, v2)
...
class MLP_end(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        super().__init__()
        hidden_size = max(input_size // 2, 64)
...
class PromptEncoder(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        self.main_model = self.main_model.to(model_device)

        typ = torch.zeros_like(input_ids, dtype=torch.long, device=model_device)
